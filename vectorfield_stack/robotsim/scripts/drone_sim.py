...
class drone_node(object):
    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            ...
            #Publish robots velocities
            # vel_msg = Twist()
            # euler_dot = euler_from_quaternion(quat_dot)
            # vel_msg.linear.x = vel_w[0]
            # vel_msg.linear.y = vel_w[1]
            # vel_msg.linear.z = vel_w[2]
            # vel_msg.angular.x = euler_dot[0]
            # vel_msg.angular.y = euler_dot[1]
            # vel_msg.angular.z = euler_dot[2]
            # self.pub_vel.publish(vel_msg)

            # Publish robot acceleration
            # accel_msg = Accel()
            # accel_msg.linear.x = acc[0]
            # accel_msg.linear.y = acc[1]
            # accel_msg.linear.z = acc[2]
    ...
    def init_node(self):
        ...
        self.pub_closest_world = rospy.Publisher("closest_point_world", Point, queue_size=1)
        self.pub_closest_body = rospy.Publisher("closest_point_body", Point, queue_size=1)
        self.pub_rviz_robot = rospy.Publisher("robot", MarkerArray, queue_size=1)
        self.pub_rviz_closest = rospy.Publisher("closest_marker", Marker, queue_size=1)
        self.pub_rviz_obst = rospy.Publisher("obstacles", MarkerArray, queue_size=1)
